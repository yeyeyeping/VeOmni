...
- `MODEL_PROCESSOR_REGISTRY` maps both `MiniMaxM3VLProcessor` (upstream) and `MiniMaxVLProcessor` (the class in the checkpoint's bundled `processing_minimax.py`) to `veomni/models/transformers/minimax_m3_vl/processing_minimax_m3_vl.py`, so training always runs the transformers>=5.12 implementation the generated modeling and the parity test are written against, whatever the checkpoint's `auto_map` resolves to. That class also handles two things the public `MiniMaxAI/MiniMax-M3` checkpoint layout otherwise breaks:
  - Its `preprocessor_config.json` declares the image/video processors *only* through a `trust_remote_code` `auto_map` (no `image_processor_type` / `video_processor_type`). `get_model_processor` strips `trust_remote_code` when it re-loads a registered processor, so the standard `from_pretrained` raises — and the loader's fallback then masks it as a misleading *"does not appear to have a file named processor_config.json"*. VeOmni's processor catches that and rebuilds from the transformers-native `MiniMaxM3VLImageProcessor` / `MiniMaxM3VLVideoProcessor`, which need no remote code.
  - The bundled `MiniMaxVLProcessor.__init__` drops `**kwargs` and so loses the `chat_template.jinja` that `from_pretrained` passes through it (checkpoints that keep the template in `tokenizer_config.json` land in the same state). VeOmni restores the template from the tokenizer, which otherwise surfaces as *"this processor does not have a chat template"* on the first sample.
