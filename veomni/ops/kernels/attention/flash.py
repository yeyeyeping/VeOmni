...
def ring_attention(query, key, value, softmax_scale, dropout_p, **kwargs):
    ...
    if query.ndim == 4 and query.shape[0] == 1:
        ...
        attn_output = ZigZagRingNPUFlashAttnVarlenFunc.apply(
            query, key, value, get_parallel_state().cp_group, cu_seq_lens_q, softmax_scale, dropout_p
        )
...
def flash_attention_forward(
    ...
    **kwargs,
    ...
    3. **Context parallelism** — decoder attention uses ring attention when CP
       is active. Independent-segment encoders may pass
       ``skip_context_parallel=True`` while retaining Ulysses.
    ...
    if getattr(get_parallel_state(), "cp_enabled", False) and not skip_context_parallel:
